...
const endpoints = [
  ...
  },
  {
    ...
    title: "Employment Screening (NIN + BVN + SSCE)",
    description: "Comprehensive employment screening that validates NIN, BVN, and SSCE (WAEC/NECO/NABTEB/NBAIS) in one request. Cross-references names and DOB across all three data sources, analyzes SSCE grades (credit = A1–C6), checks for English + Mathematics credits and 3+ other credits. Returns a 100-point score with PASS/REVIEW/FAIL decision. Sandbox returns instant results; live mode queues education via RPA (1–3 min). 15% bundle discount.",
    price: 391,
    priceNote: "NIN ₦130 + BVN ₦80 + SSCE ₦250 = ₦460 — 15% bundle discount = ₦391",
    auth: "api-key",
    async: true,
    request: {
      nin: "12345678901",
      ...
      cardPin: "1234",
      reference: "candidate_001",
      callbackUrl: "https://yourapp.com/webhook/screening"
    },
    response: {
      status: "success", code: 200, message: "Employment screening completed (sandbox)",
      data: {
        requestId: "IDC-FA4A0E07B5D5C2C1",
        reference: "candidate_001",
        status: "completed",
        decision: "PASS",
        score: 100,
        summary: "Identity: NIN ✓, BVN ✓ | Name match: NIN↔BVN ✓ (100%) | DOB match: NIN↔BVN ✓ | SSCE name match: ✓ (100%) | SSCE: 9 credit(s), English ✓ (B2), Maths ✓ (A1) | Minimum requirement (5 credits incl. English & Maths): MET",
        crossCheck: {
          ninBvnNameMatch: true, ninBvnNameScore: 100, ninBvnDobMatch: true,
          eduNameMatchesNin: true, eduNameMatchesBvn: true, eduNameScore: 100,
          eduDobMatchesNin: true, eduDobMatchesBvn: true,
          allNamesConsistent: true, allDobConsistent: true,
        },
        ssceAnalysis: {
          totalSubjects: 9, totalCredits: 9,
          englishGrade: "B2", englishIsCredit: true,
          mathGrade: "A1", mathIsCredit: true,
          otherCreditCount: 7, meetsMinimumRequirement: true,
          requirementSummary: "PASS — 9 credits including English (B2) and Mathematics (A1) plus 7 other credit subject(s). Meets minimum 5-credit requirement.",
        },
        flags: [],
        nin: { status: "verified", firstName: "JOHN", middleName: "EMEKA", lastName: "DOE", dateOfBirth: "1990-06-15", gender: "Male" },
        bvn: { status: "verified", firstName: "JOHN", middleName: "EMEKA", lastName: "DOE", dateOfBirth: "1990-06-15" },
        education: { provider: "WAEC", status: "verified", examYear: 2020, registrationNumber: "WA2020/12345", candidateName: "JOHN EMEKA DOE", candidateDateOfBirth: "1990-06-15" },
        pricing: { rawCost: 460, bundleDiscount: "15%", totalCost: 391 },
        completedAt: "2026-04-07T17:51:13.548Z"
      }
    },
    params: [
      ...
      { name: "educationProvider", type: "string", required: true, desc: "Exam body: waec, neco, nabteb, or nbais" },
      { name: "registrationNumber", type: "string", required: true, desc: "Exam registration number (WAEC, NECO, NBAIS)" },
      { name: "candidateNumber", type: "string", required: false, desc: "NABTEB candidate number (use instead of registrationNumber)" },
      { name: "examYear", type: "number", required: true, desc: "Year of examination (e.g. 2020)" },
      { name: "examType", type: "string", required: true, desc: "WAEC/NECO: Internal or GCE | NABTEB: MAY/JUN" },
      { name: "cardSerialNumber", type: "string", required: false, desc: "Scratch card serial (WAEC, NABTEB)" },
      { name: "cardPin", type: "string", required: false, desc: "Scratch card PIN (WAEC, NABTEB, NBAIS)" },
      { name: "token", type: "string", required: false, desc: "NECO verification token" },
      { name: "examMonth", type: "string", required: false, desc: "NBAIS only: MAY or NOV" },
      { name: "state", type: "string", required: false, desc: "NBAIS only: candidate state" },
      { name: "schoolName", type: "string", required: false, desc: "NBAIS only: candidate school" },
      { name: "reference", type: "string", required: false, desc: "Your internal candidate/tracking reference" },
      { name: "callbackUrl", type: "string", required: false, desc: "HTTPS webhook URL — results posted here when complete" },
    ],
    notes: [
      "Sandbox returns instant results (HTTP 200). Live mode returns HTTP 202 — poll for final result.",
      "Cross-references names and DOB across NIN, BVN, and SSCE certificate data",
      "SSCE grade analysis: credit = A1, B2, B3, C4, C5, C6. Minimum requirement: English credit + Maths credit + 3 other credits",
      "Scoring: 100-point scale — PASS (≥85), REVIEW (60–84), FAIL (<60)",
      "15% bundle discount applied (NIN ₦130 + BVN ₦80 + SSCE ₦250 = ₦460 → ₦391)",
      "Response includes crossCheck (name/DOB matches), ssceAnalysis (grade breakdown), summary, and flags",
      "callbackUrl receives POST with employment-screening.completed event when done",
    ]
  },
  {
    group: "Verification",
    method: "GET",
    path: "/verify/employment-screening/result/:requestId",
    title: "Poll Employment Screening Result",
    description: "Poll the status and full result of a previously submitted employment screening request. Returns the cross-check analysis, SSCE grade breakdown, overall score, and decision when complete.",
    price: 0,
    auth: "api-key",
    async: false,
    request: {},
    response: {
      status: "success", code: 200, message: "Employment screening completed",
      data: {
        requestId: "IDC-FA4A0E07B5D5C2C1",
        reference: "candidate_001",
        status: "completed",
        decision: "PASS",
        score: 100,
        summary: "Identity: NIN ✓, BVN ✓ | Name match: NIN↔BVN ✓ (100%) | DOB match: NIN↔BVN ✓ | SSCE: 9 credit(s), English ✓ (B2), Maths ✓ (A1)",
        crossCheck: { ninBvnNameMatch: true, ninBvnNameScore: 100, ninBvnDobMatch: true, allNamesConsistent: true, allDobConsistent: true },
        ssceAnalysis: { totalCredits: 9, englishIsCredit: true, mathIsCredit: true, meetsMinimumRequirement: true },
        flags: [],
        completedAt: "2026-04-07T17:51:13.548Z"
      }
    },
    params: [{ name: "requestId", type: "string", required: true, desc: "The IDC-prefixed requestId from POST /verify/employment-screening (URL path param)" }],
    notes: [
      "Returns status: processing while education RPA job is running, completed when done",
      "Full crossCheck, ssceAnalysis, summary, and flags included when complete",
      "This endpoint is free — no additional charge for polling",
      "If callbackUrl was set, you don't need to poll — results are pushed to your webhook",
    ]
  },
  {
    group: "Verification",
