...
import { buildScreeningReportPdf } from '../../services/screeningReportPdf';
...
router.get('/candidates/:id/pdf', screeningAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const overallAssessment = computeOverallScore(Boolean(nin?.success), Boolean(bvn?.success), ninData, bvnData, edu);

    const ninDob = normalizeDob(ninData?.dateOfBirth);
    const bvnDob = normalizeDob(bvnData?.dateOfBirth);
    const dobMatch = Boolean(ninDob && bvnDob && ninDob === bvnDob);

    const fraudFlags: string[] = (rawFraud?.flags || []).filter((f: string) =>
      !(dobMatch && /date of birth mismatch/i.test(f))
    );
    const fraud = rawFraud
      ? { ...rawFraud, flags: fraudFlags }
      : (ninData && bvnData ? computeFraudScore(ninData, bvnData) : null);

    const now = new Date().toLocaleString('en-NG', { timeZone: 'Africa/Lagos', dateStyle: 'long', timeStyle: 'short' });
    const pdfBuffer = await buildScreeningReportPdf({
      candidate: {
        ...
        educationProvider: candidate.educationProvider,
        decision: candidate.decision || overallAssessment.decision,
        overallScore: candidate.overallScore ?? overallAssessment.score,
      },
      orgName: org?.name,
      generatedAt: `${now} WAT`,
      nin,
