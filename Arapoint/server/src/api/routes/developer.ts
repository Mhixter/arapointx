...
(async () => {
  try {
    ...
    `);
    // Enterprise unified verification requests table
    await db.execute(sql`
...
// ─────────────────────────────────────────────────────────────────────────────
// ENTERPRISE UNIFIED VERIFICATION  (v2)
// POST /verify/unified
// Accepts: { reference?, callbackUrl?, identity, checks, options? }
// ─────────────────────────────────────────────────────────────────────────────
router.post('/verify/unified', apiKeyAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // ── Dynamic pricing with bundle discount ──────────────────────────────────
    let rawCost = 0;
    ...
    // ── LIVE: Persist request, return 202 immediately ─────────────────────────
    await db.execute(sql`
    ...
    setImmediate(async () => {
      try {
        ...
        if (nameScore !== null && nameScore < (options.strictNameMatch ? 0.90 : 0.72)) {
          flags.push(`Name mismatch between identity records (score: ${(nameScore * 100).toFixed(0)}%)`);
        }

        // Queue education SSCEs to RPA
        ...
        if (employment) {
          ...
          const empNameScore = nameScore ?? (fullName && ninData ? nameSimilarityScore(fullName, `${ninData.firstName} ${ninData.lastName}`) : 0.5);
        ...
        const rawScore = Math.max(0, identityPts + eduPts + empPts - fraudDeduction);
        const score = maxPts > 0 ? Math.round((rawScore / maxPts) * 100) : 100;
        ...
        // Fire webhook if callbackUrl provided and no pending RPA jobs
...
// ─── Poll Unified Result ───────────────────────────────────────────────────────
// GET /verify/unified/result/:requestId
