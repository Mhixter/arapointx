...
router.get('/performance', cacAgentAuthMiddleware, async (req: Request, res: Response) => {
  try {
    const agentId = (req as any).agentId;
    ...
    const rows = await db.execute(sql.raw(`
      SELECT
        ...
        COUNT(id) FILTER (WHERE status IN ('pending','in_progress')) as pending,
      ...
      FROM cac_registration_requests
    ...
    logger.error('CAC agent performance error', { error: error.message });
