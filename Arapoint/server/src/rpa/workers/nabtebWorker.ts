...
interface NABTEBSubject {
  subject: string;
  grade: string;
}

interface NABTEBResult {
  registrationNumber: string;
  candidateName?: string;
  examType?: string;
  examYear?: number;
  subjects: NABTEBSubject[];
...
export class NABTEBWorker extends BaseWorker {
  protected serviceName = 'nabteb_service';

  private readonly DEFAULT_SELECTORS = {
    examYearSelect: 'select[name="ExamYear"], select#ExamYear, select[name="exam_year"], select#year',
    examTypeSelect: 'select[name="ExamType"], select#ExamType, select[name="exam_type"]',
    ...
    cardSerialInput: 'input[name="SerialNumber"], input#SerialNumber, input[name="Serial"], input#Serial',
    cardPinInput: 'input[name="Pin"], input#Pin, input[name="PIN"], input#PIN, input[type="password"]',
  ...
  async execute(queryData: Record<string, unknown>): Promise<WorkerResult> {
    const data = queryData as unknown as NABTEBQueryData & { portalUrl?: string };
    logger.info('NABTEB Worker starting job', { 
    ...
    try {
      ...
      if (!portalUrl) {
        return this.createErrorResult('NABTEB portal URL not configured. Please configure in admin settings.');
      }
      ...
      logger.info('NABTEB Worker acquired browser from pool');
      ...
      logger.error('NABTEB Worker error', { error: error.message });
  ...
  private async getPortalUrl(): Promise<string | null> {
    try {
      const [setting] = await db
        .select()
        .from(adminSettings)
        .where(eq(adminSettings.settingKey, 'rpa_provider_url_nabteb'))
        .limit(1);

      return setting?.settingValue || null;
    } catch (error: any) {
      logger.error('Failed to get NABTEB portal URL', { error: error.message });
      return null;
    }
  }
  ...
  private async getCustomSelectors(): Promise<Record<string, string>> {
    try {
      const [setting] = await db
        .select()
        .from(adminSettings)
        .where(eq(adminSettings.settingKey, 'rpa_selectors_nabteb'))
        .limit(1);
      ...
      logger.warn('Failed to get custom NABTEB selectors', { error: error.message });
      return {};
    }
  }
  ...
  private async performVerification(
    page: Page,
    portalUrl: string,
    data: NABTEBQueryData,
    selectors: Record<string, string>
  ): Promise<NABTEBResult> {
    logger.info('Navigating to NABTEB portal', { url: portalUrl });
    ...
    try {
      ...
      throw new Error('Could not find form on NABTEB portal. The page may have changed.');
    }

    logger.info('Filling NABTEB form fields');
    ...
    if (data.cardPin) {
      ...
      if (!pinEntered) {
        const textInputs = await page.$$('input[type="text"], input[type="password"]');
        if (textInputs.length >= 2) {
          await textInputs[textInputs.length - 1].type(data.cardPin);
        }
      }
    }

    await this.sleep(1000);
    logger.info('Submitting NABTEB form');
    ...
    if (resultUrl === urlBeforeSubmit) {
      ...
      throw new Error('Could not submit form to NABTEB portal. Please verify your details.');
    }
    ...
    return {
      ...
      message: subjects.length > 0 ? 'NABTEB result retrieved successfully' : 'No results found',
      screenshotBase64,
    };
  }

  private async extractSubjects(page: Page): Promise<NABTEBSubject[]> {
    try {
...
export const nabtebWorker = new NABTEBWorker();
